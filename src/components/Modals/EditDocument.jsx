...
const [auxEscenary, setauxEscenary] = useState("")
  useEffect(() => {
    setloading(doc);
    setFechaVigencia(new Date("2030-12-08T05:00:00.000+00:00")); 
    const selectedId = 2;
    if (escenary.length > 0) {
      setauxEscenary(escenary[loading.id_escenario]);
    ...
    <form onSubmit={handleSubmitCreateDocument} id="myForm">
      <div className={style.selectContainer}>
        <div className={style.selectWrapper}>
          <h4>  <span className={style.numberRounded}>1</span>PASO 1: Seleccionar Tipo de Documento  </h4>
          <h5> Tipo de documento <span className={style.fieldPriority}>*</span>   </h5>
          <select className={style.customSelect} onChange={saveTipoDocument} value={tipeDocument}
          >
            <option hidden defaultValue> {loading.tipo_documento} </option> {/*shows the plan when loading */}
        ...
        <div className={style.selectWrapper}>
          <h4><span className={style.numberRounded}>2</span>PASO 2: Selecione el escenario de practicas </h4>
          <h5> Escenario de practicas{" "}<span className={style.fieldPriority}>*</span></h5>
          <select
     ...
     {/*    <option hidden defaultValue> {selectedEscenari} </option>  */}
      ...
      <center>
        <button className={style.buttonStyle} type="submit">Editar Documento  </button>
      </center>
    </form>
  );
}
